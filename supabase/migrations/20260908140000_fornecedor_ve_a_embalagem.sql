-- O fornecedor vê a embalagem que ele mesmo cobra.
--
-- A view expunha só `supplier_price`, então o Portal exibia "Seu valor
-- R$ 10,00" enquanto o vendedor via um repasse de R$ 12,00 — e o PIX saía com
-- esse valor. Mesmo pedido, dois números.
--
-- Quem confere o recebimento é o fornecedor. Conferir contra um número que o
-- próprio sistema sabe estar errado é pior do que não conferir: ele aprovaria
-- R$ 12,00 achando que eram R$ 10,00, ou recusaria um pagamento correto.
--
-- A view é recriada inteira porque não dá para acrescentar coluna a uma view
-- existente sem redefini-la. O resto é idêntico a 20260904160000.
...
select
  ...
  -- A embalagem também é dele. Sem esta coluna o Portal mostrava um valor e
  -- o vendedor pagava outro: o fornecedor conferia R$ 10,00 contra um PIX de
  -- R$ 12,00, e quem confere recebimento precisa ver o mesmo número.
  o.taxa_embalagem,
  o.status,
  o.tracking_code,
  o.etiqueta_url,
