-- Venda sem Mercado Envios, confirmada pelo próprio Mercado Livre.
--
-- POR QUE
--
-- Pedido sem envio tem duas causas opostas, e a tela dizia a mesma frase para
-- as duas: "este pedido não tem envio no Mercado Livre".
--
--   1. O envio ainda não foi criado. Some sozinho, e a etiqueta aparece.
--   2. A venda não usa Mercado Envios — frete combinado ou retirada. Nunca vai
--      ter etiqueta, e o fornecedor precisa saber para despachar por fora.
--
-- Enquanto as duas se pareciam, o fornecedor esperava por uma etiqueta que não
-- viria, e o vendedor era cobrado por algo que não dependia dele.
--
-- Esta coluna guarda a resposta do Mercado Livre quando alguém pergunta. Só é
-- marcada por conferência explícita, nunca por ausência: no momento da venda o
-- envio quase sempre ainda não existe, e marcar ali chamaria de "sem Mercado
-- Envios" quase todo pedido do sistema.

alter table public.orders
  add column if not exists sem_mercado_envios boolean not null default false;

comment on column public.orders.sem_mercado_envios is
  'Verdadeiro quando o Mercado Livre confirmou que esta venda não tem envio: frete combinado ou retirada. Nunca haverá etiqueta.';


-- ----------------------------------------------------------------------------
-- A view leva a resposta ao Portal
-- ----------------------------------------------------------------------------
-- Recriada inteira porque não dá para acrescentar coluna a uma view existente.
-- O resto é idêntico a 20260910120000.
